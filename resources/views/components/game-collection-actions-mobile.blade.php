...
@auth
    <div class="md:hidden flex justify-center items-center gap-2 mt-3"
        ...
        <button @click.stop.prevent="quickAction('playing')"
                ...
                class="min-w-[40px] min-h-[40px] w-9 h-9 rounded-full transition-all duration-200 flex items-center justify-center disabled:opacity-50 shadow-md"
                :class="currentStatus === 'playing' ? 'bg-green-500/20 ring-2 ring-green-400 text-green-400' : 'bg-gray-700 hover:bg-gray-600 text-white'"
                title="Playing">
        ...
        <button @click.stop.prevent="quickAction('played')"
                :disabled="actionLoading"
                class="min-w-[40px] min-h-[40px] w-9 h-9 rounded-full transition-all duration-200 flex items-center justify-center disabled:opacity-50 shadow-md"
                :class="currentStatus === 'played' ? 'bg-blue-500/20 ring-2 ring-blue-400 text-blue-400' : 'bg-gray-700 hover:bg-gray-600 text-white'"
                title="Played">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <button @click.stop.prevent="quickAction('backlog')"
                :disabled="actionLoading"
                class="min-w-[40px] min-h-[40px] w-9 h-9 rounded-full transition-all duration-200 flex items-center justify-center disabled:opacity-50 shadow-md"
                :class="currentStatus === 'backlog' ? 'bg-yellow-500/20 ring-2 ring-yellow-400 text-yellow-400' : 'bg-gray-700 hover:bg-gray-600 text-white'"
                title="Backlog">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <button @click.stop.prevent="quickAction('wishlist')"
                :disabled="actionLoading"
                class="min-w-[40px] min-h-[40px] w-9 h-9 rounded-full transition-all duration-200 flex items-center justify-center disabled:opacity-50 shadow-md"
                :class="isWishlisted ? 'bg-red-500/20 ring-2 ring-red-400 text-red-400' : 'bg-gray-700 hover:bg-gray-600 text-white'"
                title="Wishlist">
...
@guest
    <div class="md:hidden flex justify-center items-center gap-2 mt-3" x-data="{}" @click.stop>
        @foreach(['Playing', 'Played', 'Backlog', 'Wishlist'] as $label)
            <button @click.stop.prevent="$dispatch('open-modal', 'login-modal')"
                    class="min-w-[40px] min-h-[40px] w-9 h-9 rounded-full bg-gray-700 hover:bg-gray-600 text-white transition-all duration-200 flex items-center justify-center shadow-md"
