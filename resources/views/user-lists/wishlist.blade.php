...
@section('title', ($canManage ? 'Manage ' : '') . $user->name . "'s Wishlist")
...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        @if($canManage)
            ...
            <x-user-lists.game-search :user="$user" type="wishlist" />
        @endif

        @if($list->games->count() > 0)
            <!-- Game Grid/List -->
            @if($canManage)
                <x-user-lists.game-grid :games="$list->games" :user="$user" type="wishlist" :viewMode="$viewMode" />
            @else
                ...
                @if($viewMode === 'grid')
                    <x-featured-games-glassmorphism
                        :games="$list->games"
                        :initialLimit="999"
                        emptyMessage="No games in this wishlist."
                    />
                @else
            ...
            <div class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg">
                ...
                @if($canManage)
                    <p class="text-sm text-gray-500 dark:text-gray-500">
                        Use the search above to add games to your wishlist.
                    </p>
                @endif
            </div>
        @endif
